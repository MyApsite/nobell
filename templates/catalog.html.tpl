{# Catalog template for watches/guide/calendar/prime-residences (card-section grid). #}
<!DOCTYPE html>
<html lang="ru">
<head>
          ...
          <div class="guide">
            ...
            <div class="guide__cards">
              <div class="card-section__row">
                {% for s in items %}
                <div class="card-section__col"{% for k, v in s.card.items() %}{% if k.startswith('data_') %} data-{{ k[5:]|replace('_','-') }}="{{ v }}"{% endif %}{% endfor %}>
                  <a href="{{ s.slug }}.html" class="product">
                    <div class="product__wrap">
                      <div class="product__img"><picture><source type="image/webp" srcset="{{ s.card.card_image_base }}.webp"><img loading="lazy" src="{{ s.card.card_image_base }}.{{ s.card.card_image_ext }}" alt="{{ s.card.card_alt }}"></picture></div>
